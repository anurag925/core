...
type AsyncTask interface {
	AddTask(pattern string, handler asynq.Handler)
	StartServer() error
...
var _ AsyncTask = (*TaskClient)(nil)
...
type (
	...
	TaskClient struct {
		...
		// server runs all the tasks
		server *asynq.Server

		// mux for adding function body
		mux *asynq.ServeMux
	}
...
func NewTaskServer(conn asynq.RedisClientOpt, cfg asynq.Config) *TaskClient {
	return &TaskClient{
		server: asynq.NewServer(conn, cfg),
		mux:    asynq.NewServeMux(),
	}
...
// Starts the asynq server and starts performing the jobs
func (t *TaskClient) StartServer() error {
	return t.server.Run(t.mux)
}

// Starts the asynq server and starts performing the jobs
func (t *TaskClient) AddTask(pattern string, handler asynq.Handler) {
